...
def moment_estimator(k: int, df: pd.DataFrame):
    ...
    # select the relevant upper order statistics
...
# combining everything from above to calculate the endpoint estimator
def endpoint_estimator(k: int, df: pd.DataFrame):
    return location_estimator(k, df) - (
        scale_estimator(k, df) / moment_estimator(k, df)
    )
...
def var_ee_fixed_parameters(gamma: float, scale: float):
    return ((scale**2) * ((1 - gamma) ** 2) * (1 - 3 * gamma + 4 * gamma**2)) / (
        (gamma**4) * (1 - 2 * gamma) * (1 - 3 * gamma) * (1 - 4 * gamma)
    )
...
# print the moment_estimator for k being in between kmin% and kmax% of the sample size and 
# also plots a horizontal line for the chosen estimator (we chose the median)
# ylim is the range that will be displayed
def me_plot_ce(
    kmin: float,
    kmax: float,
    ...
    plt.axhline(y=mediangamma, lw=0.6, linestyle="--", color="dimgrey")
    plt.xlabel("k")
    plt.ylabel(r"$\hat{\gamma}_M$", rotation=0, labelpad=10)
    if ylim is not None:
        plt.ylim(ylim)
    ...
    # plt.savefig("./src/semi_parametric_method/me_plot.pdf", bbox_inches="tight")
    plt.show()
...
def ee_plot_fixed_gamma(
    ...
    plt.tick_params(axis="both", labelsize=7)
    # uncomment the following line if you want to save the image
    # plt.savefig("./src/semi_parametric_method/ee_plot_ce.pdf", bbox_inches="tight")
    plt.show()
...
def ee_plot_fixed_gamma_ce(
    ...
    if ylim is not None:
        plt.ylim(ylim)
    plt.axhline(y=medianendpoint, lw=0.5, linestyle="--", color="dimgrey")
    plt.tick_params(axis="both", labelsize=7)
    # uncomment the following line if you want to save the image
    # plt.savefig("./src/semi_parametric_method/ee_plot_ce.pdf", bbox_inches="tight")
    plt.show()
...
def ee_plot_compare_fixed_gamma(
    kmin: float, kmax: float, fix: float, df: pd.DataFrame, ylim: tuple | None = None
):
    x = np.arange(int(math.ceil(kmin * len(df))), int(kmax * math.ceil(len(df))), 1)
    plt.figure()
    ...
    plt.tick_params(axis="both", labelsize=7)
    # uncomment the following line if you want to save the image
    # plt.savefig("./src/semi_parametric_method/eecompare.pdf", bbox_inches="tight")
    plt.show()


#Here start the actual calculations


# load the combined and smoothed data. The csv file can be generated using the combine_and_smooth file
...
print(
    "The median estimate for the moment estimator is given by: " + str(mediangamma)
)  # -0.2308862817137589
...
# calculate the median endpoint in the chosen interval 107 to 190
ks_endpoint = np.arange(107, 190)
y = np.array(
    [endpoint_estimator_fixed_gamma(k, mediangamma, df_25_sorted) for k in ks_endpoint]
)
medianendpoint = np.median(y)
print(
    "The median of the endpoint estimates is given by: " + str(medianendpoint)
)  # 124.7137107826764
...
print(
    "The standard error using the fixed mediangamma estimate is given by: " + str(se)
)  # 2.759392291561523

# calcuate the standarderror using moment estimator with k = 136
se_2 = math.sqrt(
    var_ee_fixed_parameters(moment_estimator(136, df_25_sorted), scale) / 136
)
print(
    "The standard error using the moment estimator is given by: " + str(se_2)
)  # 2.5643850886638475

# give upper confidence bound for fixed estimate of gamma
upperbound = medianendpoint + 1.96 * se
print(
    "The upper 0.95 confidence bound using the fixed mediangamma estimate is given by: "
    + str(upperbound)
)  # 130.122119674137
